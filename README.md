...
The commands describe themselves in English even here — the picker is where you choose a command before any reply exists, and one that switches language mid-list is harder to scan. Only the replies and the morning report are Korean. `Status` and `Category` are both English in the database — they are fixed choices, which makes them interface rather than data. Item names stay in the language they were listed in.
