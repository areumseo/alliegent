...
`/add … cal: true` mirrors the item into `ICLOUD_WRITE_CALENDAR`, as a 30-minute event when it has a time and an all-day entry when it does not. **Nothing goes into the calendar unless asked.** It used to follow the time — an item at an hour went in, a bare task stayed out — on the reasoning that something happening at an hour is what a calendar is for. In practice most timed rows are shifts and classes that are *already* in the calendar, so the rule wrote a second copy of them and each one had to be deleted twice. Typing `cal: true` on the few items that are genuinely new appointments is less work than that, and the distinction between the two tools survives either way: a chore belongs on a list, and a calendar filled with chores stops showing what the day is committed to.
...
Second-hand sales live in two Notion databases of their own — **Karrot Items**, everything being sold, and **Karrot Expenses**, what selling costs and what was bought — and in their own channel, and **this one channel is in Korean**. Every item name and category in that database is Korean already; a listing translated into "Clothing" is one the reader cannot search for. The slash commands keep ASCII names so nothing needs an input-method switch — only what they say is Korean.
...
**Selling costs come off revenue.** Neighbourhood ads and packaging are recorded in Karrot Expenses rather than in Karrot Items — an expense has no price, no buyer and no status, so keeping it as a row in the item list would mean teaching every count to skip it, and giving it a number in `/karrot list`. `/karrot spent <amount> <kind>` records one; `sales` and the Saturday report then show revenue and net side by side, with the cost named once underneath rather than repeated as a column against every period — it is the same few purchases being divided up over and over, and a column of them buries the two numbers being compared. Without `NOTION_KARROT_EXPENSES_DB_ID` the revenue reported is gross, and no net figure is shown at all.

**What you buy on Karrot is tracked, and deliberately kept out of net.** `/karrot bought <item> <amount>` files a purchase in Karrot Expenses too, under the `Purchase` kind, and the Saturday report gives it its own line: `Bought on Karrot: ₩X this week, ₩Y all time. Not in Net.` Packaging is what a sale cost to make and a purchase is not, so a single net covering both would move without saying which half moved it — a week of buying would read as a week that sold badly. It is a separate command rather than a third choice on `/karrot spent` for the same reason: the two are one mistyped option apart, and the mistake is invisible once it is in.
