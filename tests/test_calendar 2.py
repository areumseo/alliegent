...
def test_a_timed_event_keeps_its_local_clock_time():
    ...
    assert [e.summary for e in events] == ["Dance class"]
...
def test_a_weekly_rule_is_expanded_onto_later_days():
    ...
    assert _events_from(ics(WEEKLY), date(2026, 8, 25), SEOUL)[0].summary == "Dance class"
...
def test_a_cancelled_occurrence_disappears():
    ...
    cancelled = WEEKLY.replace(
        "SUMMARY:Dance class", "EXDATE;TZID=Asia/Seoul:20260811T191000\nSUMMARY:Dance class"
    )
...
def test_all_day_events_sort_ahead_of_timed_ones():
    timed = Event("Dance class", datetime(2026, 8, 10, 19, 10, tzinfo=SEOUL), all_day=False)
    ...
    assert [e.summary for e in ordered] == ["Holiday", "Call", "Dance class"]
